...
const CategoryInsert = () => {
    ...
    const listCrumb = [{ name: "Home", navigateTo: "/" }, { name: "Categorias", navigateTo: CategoryRoutesEnum.CATEGORY }, { name: "Inserir Categoria" }]
        ...
        <Screen listCrumb={listCrumb}>
            <DisplayFlex directionwrap="row nowrap" background="#" justify="center">
                <LimitedContainer width="400px" directionwrap="column nowrap" gap="15px" align="flex-end">
                    <Input onChange={(event) => onChange(event)} value={category.name} campo="Nome da Categoria" />
                    <DisplayFlex width="100%" background="#" gap="10px" justify="flex-end">
                        <Button onClick={clickInsertCategory} loading={loading} disabled={disableButton} width="135px" type="primary">Inserir Categoria</Button>
